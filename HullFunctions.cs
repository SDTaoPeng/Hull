...
namespace ConvexHull_1
{
    public class HullFunctions
    {
        ...
        public static int[] getBoundary(Line line, int threshold)
        {
            int[] boundary = new int[4];
            Node aNode = line.nodes[0];
            Node bNode = line.nodes[1];
            int min_x_position = (int)Math.Floor(Math.Min(aNode.x, bNode.x) / threshold);
            int min_y_position = (int)Math.Floor(Math.Min(aNode.y, bNode.y) / threshold);
            int max_x_position = (int)Math.Floor(Math.Max(aNode.x, bNode.x) / threshold);
            int max_y_position = (int)Math.Floor(Math.Max(aNode.y, bNode.y) / threshold);
        ...
        public static List<Node> getNearbyPoints(Line line, List<Node> nodeList, int threshold)
        {
            ...
            {
                boundary = getBoundary(line, threshold);
                foreach (Node node in nodeList)
                {
                    if (!(node.x == line.nodes[0].x && node.y == line.nodes[0].y ||
                        node.x == line.nodes[1].x && node.y == line.nodes[1].y))
                    {
                        node_x_rel_pos = (int)Math.Floor(node.x / threshold);
                        node_y_rel_pos = (int)Math.Floor(node.y / threshold);
